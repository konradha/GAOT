...
class ModelArgsConfig:
    ...
    magno: MAGNOConfig = field(
        default_factory=MAGNOConfig
    )  # MAGNO encoder/decoder config
...
class ModelConfig:
    ...
    latent_tokens_size: Tuple[int, int] = (
        64,
        64,
    )  # Latent token dimensions (H,W) or (H,W,D)
    args: ModelArgsConfig = field(
        default_factory=ModelArgsConfig
    )  # Model component configurations
...
class DatasetConfig:
    ...
    coord_scaling: str = (
        "per_dim_scaling"  # Coordinate scaling: ["global_scaling", "per_dim_scaling"]
    )
    ...
    predict_mode: str = (
        "all"  # Inference mode: ["all", "autoregressive", "direct", "star"]
    )
...
class OptimizerConfig:
    ...
    args: OptimizerArgsConfig = field(
        default_factory=OptimizerArgsConfig
    )  # Optimizer arguments
