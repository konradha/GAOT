...
class BaseTrainer(ABC):
    ...
    def __init__(self, config):
        ...
        elif self.setup_config.dtype in [
            "double",
            "torch.float64",
            "torch.DoubleTensor",
        ]:
    ...
    def init_optimizer(self, optimizer_config):
        ...
        optimizer_map = {"adam": AdamOptimizer, "adamw": AdamWOptimizer}
    ...
    def plot_losses(
        ...
        if val_losses is None:
            ...
            if (np.array(losses) > 0).all():
                ax.set_yscale("log")
            np.savez(
                self.path_config.loss_path[:-4] + ".npz", epochs=epochs, losses=losses
            )
            ...
            ax[0].scatter(
                [best_epoch], [best_loss], c="r", marker="o", label="best loss"
            )
