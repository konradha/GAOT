...
def save_ckpt(path, **kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if isinstance(
            v, (torch.nn.DataParallel, torch.nn.parallel.DistributedDataParallel)
        ):
            kwargs[k] = (
                v.state_dict()
            )  # save the wrapped model, includding the 'module.' prefix
        else:
...
def load_ckpt(path, **kwargs):
    ...
    for k, v in kwargs.items():
        ...
        if all(key.startswith("module.") for key in ckpt_keys) and not any(
        ...
        elif not any(key.startswith("module.") for key in ckpt_keys) and all(
            key.startswith("module.") for key in model_keys
        ):
            new_state_dict = {}
            for key in ckpt_keys:
...
def compute_sequential_stats(
    u_data: np.ndarray,
    c_data: Optional[np.ndarray],
    t_values: np.ndarray,
    metadata,
    ...
    if (
        use_metadata_stats
        and hasattr(metadata, "u_mean")
        and hasattr(metadata, "u_std")
    ):
        stats["u"] = {
    ...
    if c_data is not None:
        if (
            use_metadata_stats
            and hasattr(metadata, "c_mean")
            and hasattr(metadata, "c_std")
        ):
            stats["c"] = {
    ...
    if use_time_norm:
        ...
        for lag in range(
            time_step, max_time_diff + 1, time_step
        ):  # Even lags from time_step to max_time_diff
