...
class OptimizerArgsConfig:
    ...
    scheduler: str = (
        "mix"  # Learning rate scheduler type, support ['step', 'cos', 'exp', 'mix']
    )
    early_save_metric: str = (
        "val"  # Metric for early stopping, support ['train', 'val']
    )
    # for mix scheduler
    ...
    scheduler_gamma: float = (
        0.8  # Multiplicative factor for learning rate decay in StepLR scheduler
    )
    ...
    scheduler_eta_min: float = (
        1e-4  # Minimum learning rate for CosineAnnealingLR scheduler
    )
    ...
    def get_lr(self):
        if self.last_epoch < self.warmup_epochs:
            ...
            lr = self.initial_lr + (self.max_lr - self.initial_lr) * (
                self.last_epoch / max(1, self.warmup_epochs - 1)
            )
