...
def rescale(
    data: torch.Tensor, target_range: Tuple[float, float] = (-1, 1)
) -> torch.Tensor:
    """
...
class CoordinateScaler:
    ...
    def __init__(
        self, target_range: Tuple[float, float] = (-1, 1), mode: str = "per_dim_scaling"
    ):
        """
    ...
    def fit(self, coords: torch.Tensor):
        ...
        if self.mode == "global_scaling":
            ...
            coord_range = torch.where(
                coord_range == 0, torch.ones_like(coord_range), coord_range
            )
...
class MinMaxScaler:
    ...
    def transform(self, data: torch.Tensor) -> torch.Tensor:
        ...
        return (data - self.data_min.to(data.device)) * self.scale.to(
            data.device
        ) + feature_min
    ...
    def inverse_transform(self, data: torch.Tensor) -> torch.Tensor:
        ...
        feature_min, _ = self.feature_range
        return (data - feature_min) / self.scale.to(data.device) + self.data_min.to(
            data.device
        )
