...
def compute_batch_errors(
    gtr: torch.Tensor, prd: torch.Tensor, metadata: Metadata
) -> torch.Tensor:
    """
    ...
    mean = torch.tensor(metadata.global_mean, device=gtr.device, dtype=gtr.dtype)[
        active_vars
    ].reshape(1, 1, 1, -1)
    std = torch.tensor(metadata.global_std, device=gtr.device, dtype=gtr.dtype)[
        active_vars
    ].reshape(1, 1, 1, -1)
    ...
    chunks = torch.tensor(
        adjusted_chunks, device=gtr.device, dtype=torch.long
    )  # Shape: [var]
    ...
    chunks_expanded = chunks.unsqueeze(0).expand(
        error_sum.size(0), -1
    )  # Shape: [batch_size, var]
    error_per_chunk = torch.zeros(
        error_sum.size(0), num_chunks, device=gtr.device, dtype=error_sum.dtype
    )
    ...
    gtr_sum_per_chunk = torch.zeros(
        gtr_abs_sum.size(0), num_chunks, device=gtr.device, dtype=gtr_abs_sum.dtype
    )
    gtr_sum_per_chunk.scatter_add_(1, chunks_expanded, gtr_abs_sum)

    # compute relative errors per chunk
    relative_error_per_chunk = error_per_chunk / (
        gtr_sum_per_chunk + EPSILON
...
def compute_final_metric(all_relative_errors: torch.Tensor) -> float:
    ...
    median_error_per_chunk = torch.median(all_relative_errors, dim=0)[
        0
    ]  # Shape: [num_chunks]
