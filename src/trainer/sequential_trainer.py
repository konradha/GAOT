...
class SequentialTrainer(BaseTrainer):
    ...
    def _init_variable_coords_mode(self, data_splits):
        ...
        loader_kwargs = {
            "encoder_graphs": {
                ...
                if all_graphs.get("train")
                else None,
                "val": all_graphs["val"]["encoder"] if all_graphs.get("val") else None,
            ...
            "decoder_graphs": {
                ...
                "val": all_graphs["val"]["decoder"] if all_graphs.get("val") else None,
    ...
    def init_model(self, model_config):
        ...
        print(
            f"Initialized {model_config.name} model for sequential data with {self.coord_dim}D coordinates"
        )
    ...
    def _call_model_autoregressive_predict(
        ...
        if self.coord_mode == "fx":
            ...
            raise NotImplementedError(
                "Variable coordinates autoregressive prediction not yet implemented"
            )
        ...
        return self.model.autoregressive_predict(
            ...
            use_conditional_norm=getattr(
                self.model_config, "use_conditional_norm", False
            ),
        )
    ...
    def test(self):
        ...
        if (
            hasattr(self.dataset_config, "predict_mode")
            and self.dataset_config.predict_mode == "all"
        ):
        ...
        for mode in modes:
            ...
            with torch.no_grad():
                pbar = tqdm(
                    total=len(test_loader), desc=f"Testing ({mode})", colour="blue"
                )
                ...
                for i, batch in enumerate(test_loader):
                    ...
                    pred = self._call_model_autoregressive_predict(
                        x_batch, time_indices, coord_batch if len(batch) == 3 else None
                    )
                    ...
                    if example_data is None:
                        coord_batch_for_plot = (
                            coord_batch
                            if self.coord_mode == "vx" and len(batch) == 3
                            else None
                        )
                        example_data = self._prepare_example_data(
                            x_batch, y_batch, pred, time_indices, coord_batch_for_plot
                        )
    ...
    def _prepare_example_data(
        self, x_batch, y_batch, pred, time_indices, coord_batch=None
    ):
        ...
        if c_dim > 0:
            x_u_part = (
                x_batch[..., :u_dim].cpu() * self.stats["u"]["std"]
                + self.stats["u"]["mean"]
            )
            x_c_part = (
                x_batch[..., u_dim : u_dim + c_dim].cpu() * self.stats["c"]["std"]
                + self.stats["c"]["mean"]
            )
    ...
    def _store_test_results(self, errors_dict, modes):
        ...
        if len(modes) > 1:
            self.config.datarow["relative error (direct)"] = errors_dict.get(
                "direct", 0.0
            )
            self.config.datarow["relative error (auto2)"] = errors_dict.get(
                "autoregressive", 0.0
            )
    ...
    def _plot_test_results(self, example_data):
        ...
        try:
            if self.metadata.names["c"] and "c" in self.stats:
                ...
                gt_with_c = np.stack(
                    [example_data["gt_sequence"][-1], example_data["input"][..., -1]],
                    axis=-1,
                ).squeeze(1)
                pred_with_c = np.stack(
                    [example_data["pred_sequence"][-1], example_data["input"][..., -1]],
                    axis=-1,
                ).squeeze(1)
    ...
    def _create_animation(self, example_data):
        ...
        try:
            ...
            gt_sequence = example_data[
                "gt_sequence"
            ]  # [n_timesteps, n_points, n_channels]
            pred_sequence = example_data[
                "pred_sequence"
            ]  # [n_timesteps, n_points, n_channels]
            ...
            if len(time_indices) > 1:
                time_values = [
                    t_values_full[idx] for idx in time_indices[1:]
                ]  # Skip initial time
            else:
            ...
            create_sequential_animation(
                ...
                interval=800,
                symmetric=self.metadata.signed["u"]
                if self.metadata.signed.get("u")
                else [True],
                domain=self.metadata.domain_x
                if hasattr(self.metadata, "domain_x")
                else None,
                names=self.metadata.names["u"]
                if self.metadata.names.get("u")
                else None,
                colorbar_type="light",
                show_error=True,
            )
