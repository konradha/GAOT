...
class GraphBuilder:
    ...
    def build_all_graphs(
        ...
        if "test" in data_splits:
            ...
            all_graphs["test"] = {"encoder": encoder_test, "decoder": decoder_test}
        ...
        if build_train:
            ...
            if "val" in data_splits:
                ...
                all_graphs["val"] = {"encoder": encoder_val, "decoder": decoder_val}
        else:
    ...
    def validate_graphs(self, graphs: dict, expected_samples: dict):
        ...
        for split_name, split_graphs in graphs.items():
            ...
            for i, (enc_sample, dec_sample) in enumerate(
                zip(encoder_graphs, decoder_graphs)
            ):
                assert isinstance(enc_sample, list), (
                    f"Encoder sample {i} should be list of scales"
                )
                assert isinstance(dec_sample, list), (
                    f"Decoder sample {i} should be list of scales"
                )
...
class CachedGraphBuilder(GraphBuilder):
    ...
    def __init__(
        self, neighbor_search_method: str = "auto", cache_dir: Optional[str] = None
    ):
        super().__init__(neighbor_search_method)
        self.cache_dir = cache_dir

    def _get_cache_path(
        self, dataset_name: str, split_name: str, graph_type: str
    ) -> str:
        ...
        return os.path.join(
            self.cache_dir, f"{dataset_name}_{split_name}_{graph_type}_graphs.pt"
        )
