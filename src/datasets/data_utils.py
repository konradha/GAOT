...
class CustomDataset(Dataset):
    ...
    def __init__(
        ...
        if len(encoder_graphs) != n_samples:
            raise ValueError(
                "encoder_graphs and u_data must have same number of samples"
            )
        if len(decoder_graphs) != n_samples:
            raise ValueError(
                "decoder_graphs and u_data must have same number of samples"
            )
...
class DynamicPairDataset(Dataset):
    ...
    def _generate_time_pairs(self, num_timesteps: int, time_step: int):
        ...
        self.time_diffs = (
            self.t_values[self.t_out_indices] - self.t_values[self.t_in_indices]
        )
        ...
        if self.use_time_norm and self.stats is not None:
            ...
            self.start_times_norm = (
                self.start_times - self.stats["start_time"]["mean"]
            ) / self.stats["start_time"]["std"]
            self.time_diffs_norm = (
                self.time_diffs - self.stats["time_diffs"]["mean"]
    ...
    def _prepare_time_features(self):
        ...
        self.start_time_expanded = self.start_times_norm.unsqueeze(1).expand(
            -1, self.num_nodes
        )
        self.time_diff_expanded = self.time_diffs_norm.unsqueeze(1).expand(
            -1, self.num_nodes
        )
...
class TestDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.c_data is not None:
            ...
            if self.stats is not None and "c" in self.stats:
                c_start_norm = (c_start - self.stats["c"]["mean"]) / self.stats["c"][
                    "std"
                ]
            else:
        ...
        target_sequence = self.u_data[
            idx, self.time_indices[1:]
        ]  # [n_timesteps-1, num_nodes, num_vars]
