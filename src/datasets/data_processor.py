...
class DataProcessor:
    ...
    def _load_raw_data(self) -> Dict:
        ...
        return {"u": u_array, "c": c_array, "x": x_array}
    ...
    def _load_coordinate_data(self, ds: xr.Dataset, u_array: np.ndarray) -> np.ndarray:
        ...
        if self.metadata.group_x is not None:
            ...
            if self.metadata.domain_x is None:
                raise ValueError(
                    "Either group_x or domain_x must be specified in metadata"
                )
            ...
            x_array = np.repeat(
                x_array, num_samples, axis=0
            )  # [num_samples, 1, num_nodes, 2]
    ...
    def _split_and_normalize_data(
        ...
        # Validate shapes
        assert u_array.shape[1] == 1, (
            "Expected num_timesteps to be 1 for static datasets"
        )
    ...
    def _compute_and_apply_normalization(
        self, u_train, u_val, u_test, c_train, c_val, c_test
    ):
