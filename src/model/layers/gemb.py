...
# Node Pos Encoding
######################
def node_pos_encode(x: torch.Tensor, freq: int = 4) -> torch.Tensor:
    """
    Parameters
    ----------
    x: torch.Tensor
    ...
    phi = np.pi * (x + 1)
    x = (
        freqs[None, :, None] * phi[:, None, :]
    )  # [n_points, 1, dim] * [1, freq, 1] -> [n_points, freq, dim]
...
class GeometricEmbedding(nn.Module):
    def __init__(
        ...
        if self.method == "statistical":
            ...
            self.fc = nn.Sequential(nn.Linear(64, output_dim), nn.ReLU())
        else:
    ...
    def _compute_statistical_features(self, input_geom, latent_queries, spatial_nbrs):
        ...
        neighbors_index = spatial_nbrs[
            "neighbors_index"
        ]  # Shape: [num_total_neighbors]
        neighbors_row_splits = spatial_nbrs[
            "neighbors_row_splits"
        ]  # Shape: [num_queries + 1]

        num_neighbors_per_query = (
            neighbors_row_splits[1:] - neighbors_row_splits[:-1]
        )  # Shape: [num_queries]
        query_indices_per_neighbor = torch.repeat_interleave(
            torch.arange(num_queries, device=device), num_neighbors_per_query
        )
        # Shape: [num_total_neighbors]

        nbr_coords = input_geom[
            neighbors_index
        ]  # Shape: [num_total_neighbors, num_dims]
        query_coords_per_neighbor = latent_queries[
            query_indices_per_neighbor
        ]  # Shape: [num_total_neighbors, num_dims]

        ## --- 1. compute distances ---
        distances = torch.norm(
            nbr_coords - query_coords_per_neighbor, dim=1
        )  # Shape: [num_total_neighbors]
        ...
        D_avg = scatter_mean(
            distances, query_indices_per_neighbor, dim=0, dim_size=num_queries
        )  # Shape: [num_queries,]

        # --- 3. compute variance of distances ---
        distances_squared = distances**2
        E_X2 = scatter_mean(
            distances_squared, query_indices_per_neighbor, dim=0, dim_size=num_queries
        )  # Shape: [num_queries,]
        E_X = D_avg
        ...
        nbr_centroid = scatter_mean(
            nbr_coords, query_indices_per_neighbor, dim=0, dim_size=num_queries
        )  # Shape: [num_queries, num_dims]
        Delta = nbr_centroid - latent_queries

        # --- 5. compute covariance matrix of neighbors ---
        nbr_coords_centered = (
            nbr_coords - nbr_centroid[query_indices_per_neighbor]
        )  # Shape: [num_total_neighbors, num_dims]
        cov_components = nbr_coords_centered.unsqueeze(
            2
        ) * nbr_coords_centered.unsqueeze(
            1
        )  # Shape: [num_total_neighbors, num_dims, num_dims]
        cov_sum = scatter_sum(
            cov_components, query_indices_per_neighbor, dim=0, dim_size=num_queries
        )  # Shape: [num_queries, num_dims, num_dims]
        N_i_clamped = N_i.clone()
        N_i_clamped[N_i_clamped == 0] = 1.0
        cov_matrix = cov_sum / N_i_clamped.view(
            -1, 1, 1
        )  # Shape: [num_queries, num_dims, num_dims]
        ...
        if has_neighbors.any():
            # covariance matrices
            cov_matrix_valid = cov_matrix[
                has_neighbors
            ]  # Shape: [num_valid_queries, num_dims, num_dims]
            eigenvalues = torch.linalg.eigvalsh(
                cov_matrix_valid
            )  # Shape: [num_valid_queries, num_dims]
    ...
    def _compute_pointnet_features(self, input_geom, latent_queries, spatial_nbrs):
        ...
        if has_neighbors.any():
            ...
            query_indices_per_neighbor = torch.repeat_interleave(
                valid_query_indices, num_neighbors_per_query[has_neighbors]
            )
            nbr_coords = input_geom[neighbors_index]  # [num_total_neighbors, num_dims]
            query_coords_per_neighbor = latent_queries[
                query_indices_per_neighbor
            ]  # [num_total_valid_neighbors, num_dims]

            nbr_coords_centered = (
                nbr_coords - query_coords_per_neighbor
            )  # [num_total_valid_neighbors, num_dims]
            nbr_features = self.pointnet_mlp(
                nbr_coords_centered
            )  # [num_total_valid_neighbors, feature_dim]
            ...
            pointnet_features = pooled_features[
                valid_query_indices
            ]  # [num_valid_queries, feature_dim]
            pointnet_features = self.fc(
                pointnet_features
            )  # [num_valid_queries, output_dim]
    ...
    def forward(self, input_geom, latent_queries, spatial_nbrs):
        if self.method == "statistical":
            geo_features_normalized = self._compute_statistical_features(
                input_geom, latent_queries, spatial_nbrs
            )
            return self.mlp(geo_features_normalized)
        elif self.method == "pointnet":
            return self._compute_pointnet_features(
                input_geom, latent_queries, spatial_nbrs
            )
        else:
            raise ValueError(f"Unknown method: {self.method}")
