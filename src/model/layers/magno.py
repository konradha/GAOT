...
class MAGNOConfig:
    ...
    embedding_method: str = (
        "statistical"  # Geometric embedding method, support ['statistical', 'pointnet']
    )
    pooling: str = (
        "max"  # Pooling for pointnet embedding, support ['max', 'mean', 'sum']
    )
    ...
    neighbor_strategy: str = (
        "radius"  # Neighbor search strategy, support ['radius', 'knn']
    )
...
class MAGNOEncoder(nn.Module):
    ...
    def __init__(self, in_channels: int, out_channels: int, config: MAGNOConfig):
        ...
        if self.use_geoembed:
            ...
            self.recovery = ChannelMLP(
                in_channels=2 * out_channels, out_channels=out_channels, n_layers=1
            )
    ...
    def _detect_coordinate_mode(self, x_coord: torch.Tensor) -> Literal["fx", "vx"]:
        ...
        if x_coord.ndim == 2:
            ...
            raise ValueError(
                f"x_coord must be 2D or 3D tensor, got shape {x_coord.shape}"
            )

    def _compute_neighbors(
        self,
        x_coord: torch.Tensor,
        latent_coord: torch.Tensor,
        mode: Literal["fx", "vx"],
    ) -> List:
    ...
    def forward(
        ...
        if coord_mode == "fx":
            if x_coord.shape[1] != self.coord_dim:
                raise ValueError(
                    f"Expected x_coord shape [num_nodes, {self.coord_dim}], got {x_coord.shape}"
                )
            num_nodes = x_coord.shape[0]
        else:  # vx mode
            if x_coord.shape[0] != batch_size or x_coord.shape[2] != self.coord_dim:
                raise ValueError(
                    f"Expected x_coord shape [{batch_size}, num_nodes, {self.coord_dim}], got {x_coord.shape}"
                )
            num_nodes = x_coord.shape[1]

        if pndata.shape[:2] != (batch_size, num_nodes):
            raise ValueError(
                f"pndata shape mismatch: expected [{batch_size}, {num_nodes}, in_channels], got {pndata.shape}"
            )

        if latent_tokens_coord.shape[1] != self.coord_dim:
            raise ValueError(
                f"Expected latent_tokens_coord shape [num_latent, {self.coord_dim}], got {latent_tokens_coord.shape}"
            )
        ...
        pndata = self.lifting(pndata).permute(
            0, 2, 1
        )  # Back to [batch, nodes, channels]

        # --- Prepare scale weights if enabled ---
        if self.use_scale_weights:
            scale_weights = self.scale_weighting(
                latent_tokens_coord
            )  # [num_latent, num_scales]
        ...
        if len(encoded_scales) == 1:
            ...
            if self.use_scale_weights:
                ...
                for i, enc in enumerate(encoded_scales):
                    weights = scale_weights[:, i : i + 1].unsqueeze(
                        0
                    )  # [1, num_latent, 1]
                    encoded += weights * enc
            else:
...
class MAGNODecoder(nn.Module):
    ...
    def _detect_coordinate_mode(self, query_coord: torch.Tensor) -> Literal["fx", "vx"]:
        ...
        if query_coord.ndim == 2:
            ...
            raise ValueError(
                f"query_coord must be 2D or 3D tensor, got shape {query_coord.shape}"
            )
    ...
    def _compute_neighbors(
        ...
        if mode == "fx":
            ...
            for scale in self.scales:
                scaled_radius = self.config.radius * scale
                neighbors = self.nb_search(
                    data=latent_coord, queries=query_coord, radius=scaled_radius
                )
                neighbors_per_scale.append(neighbors)
    ...
    def forward(
        ...
        if coord_mode == "fx":
            if query_coord.shape[1] != self.coord_dim:
                raise ValueError(
                    f"Expected query_coord shape [num_query, {self.coord_dim}], got {query_coord.shape}"
                )
            num_query = query_coord.shape[0]
        else:  # vx mode
            if (
                query_coord.shape[0] != batch_size
                or query_coord.shape[2] != self.coord_dim
            ):
                raise ValueError(
                    f"Expected query_coord shape [{batch_size}, num_query, {self.coord_dim}], got {query_coord.shape}"
                )
        ...
        decoded = self.projection(decoded).permute(
            0, 2, 1
        )  # Back to [batch, query, channels]
