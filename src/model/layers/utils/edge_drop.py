...
def apply_edge_drop_csr(
    ...
    if sampling_strategy == "ratio":
        ...
        target_node_indices = torch.arange(
            num_target_nodes, device=device
        ).repeat_interleave(num_reps)
        ...
        new_num_reps = torch.bincount(
            sampled_target_node_indices, minlength=num_target_nodes
        )
        sampled_neighbors_row_splits = torch.zeros(
            num_target_nodes + 1, dtype=neighbors_row_splits.dtype, device=device
        )
        ...
        keep_mask = torch.ones(
            num_total_original_edges, dtype=torch.bool, device=device
        )
        ...
        for i in nodes_to_sample_idx:
            ...
            local_keep_mask = torch.zeros(
                num_node_neighbors, dtype=torch.bool, device=device
            )
