...
class GAOT(nn.Module):
    ...
    def __init__(
        self, input_size: int, output_size: int, config: Optional[dataclass] = None
    ):
        ...
        self.encoder = self.init_encoder(
            input_size, self.node_latent_size, config.args.magno
        )
        self.processor = self.init_processor(
            self.node_latent_size, config.args.transformer
        )
        self.decoder = self.init_decoder(
            output_size, self.node_latent_size, config.args.magno
        )

    def init_encoder(self, input_size, latent_size, config):
        return MAGNOEncoder(
            in_channels=input_size, out_channels=latent_size, config=config
        )
    ...
    def init_processor(self, node_latent_size, config):
        ...
        self.patch_linear = nn.Linear(
            patch_volume * node_latent_size, patch_volume * node_latent_size
        )
    ...
    def init_decoder(self, output_size, latent_size, config):
        return MAGNODecoder(
            in_channels=latent_size, out_channels=output_size, config=config
        )
    ...
    def autoregressive_predict(
        ...
        for idx in range(1, num_timesteps):
            ...
            start_time_expanded = torch.full(
                (batch_size, num_nodes, 1),
                start_time_norm,
                dtype=x_batch.dtype,
                device=x_batch.device,
            )
            time_diff_expanded = torch.full(
                (batch_size, num_nodes, 1),
                time_diff_norm,
                dtype=x_batch.dtype,
                device=x_batch.device,
        ...
        return torch.stack(
            predictions, dim=1
        )  # [batch_size, num_timesteps-1, num_nodes, output_dim]
    ...
    def _process_autoregressive_prediction(
        ...
        if stepper_mode == "output":
            ...
            current_u_denorm = current_u * u_std + u_mean
            time_diff_tensor = torch.tensor(
                time_diff, dtype=pred.dtype, device=pred.device
            )
