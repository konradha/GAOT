...
@torch.no_grad()
def interpolate_timestep_gpu_fast(query_pts, source_x, source_tri, source_mask, source_fields, device):
    ...
    field_tensors = {vn: torch.from_numpy(f.astype(np.float32)).to(device) for vn, f in source_fields.items()}
    ...
    result_fields = {vn: torch.zeros(n_query, device=device, dtype=torch.float32) for vn in var_names}
    ...
    for start in range(0, n_query, chunk_size):
        ...
        for vn in var_names:
            interp = w_vals * f0[vn][tri_idx] + u_vals * f1[vn][tri_idx] + v_vals * f2[vn][tri_idx]
...
def process_sample_gpu(sample, merge_tol, device):
    ...
    fields = {vn: np.zeros((n_timesteps, n_nodes), dtype=np.float32) for vn in var_names}
    ...
    for t in [0, sample["x"].shape[0] // 2, sample["x"].shape[0] - 1]:
        ...
        for vn in var_names:
            ...
            if valid.sum() > 0:
                ...
                errors[vn].append({"l1": diff.mean(), "l2": np.sqrt((diff**2).mean()), "linf": diff.max()})
    
    return {vn: {"l1": np.mean([e["l1"] for e in errs]), "l2": np.mean([e["l2"] for e in errs]), "linf": np.max([e["linf"] for e in errs])} for vn, errs in errors.items() if errs}
...
def write_vtkhdf(path, all_results, sample_ids):
    ...
    sample_node_offsets = np.cumsum([0] + [len(r["grid"]) for r in all_results]).astype(np.int64)
    sample_tri_offsets = np.cumsum([0] + [len(r["triangles"]) for r in all_results]).astype(np.int64)
    ...
    with h5py.File(path, "w") as f:
        ...
        print("  Connectivity...")
        conn = np.concatenate([(r["triangles"] + sample_node_offsets[i]).flatten() for i, r in enumerate(all_results)]).astype(np.int64)
        vhdf.create_dataset("Connectivity", data=conn, compression="gzip", compression_opts=1)
        del conn
        
        vhdf.create_dataset("Offsets", data=(np.arange(1, total_triangles + 1) * 3).astype(np.int64), compression="gzip", compression_opts=1)
        vhdf.create_dataset("Types", data=np.full(total_triangles, 5, dtype=np.uint8), compression="gzip", compression_opts=1)
        vhdf.create_dataset("NumberOfPoints", data=np.array([total_nodes], dtype=np.int64))
        vhdf.create_dataset("NumberOfCells", data=np.array([total_triangles], dtype=np.int64))
        vhdf.create_dataset("NumberOfConnectivityIds", data=np.array([total_triangles * 3], dtype=np.int64))
        ...
        pdata.create_dataset("drone_mask", data=arr, compression="gzip", compression_opts=1)
        del arr
        ...
        steps.create_dataset("NumberOfParts", data=np.ones(total_timesteps, dtype=np.int64))
        steps.create_dataset("PointOffsets", data=np.array(point_offs, dtype=np.int64).reshape(-1, 1))
        steps.create_dataset("CellOffsets", data=np.zeros((total_timesteps, 1), dtype=np.int64))
        steps.create_dataset("ConnectivityIdOffsets", data=np.zeros((total_timesteps, 1), dtype=np.int64))
        
        f.create_dataset("SampleOffset", data=(np.arange(n_samples) * n_timesteps).astype(np.int64))
        ...
        f.create_dataset("SampleIDs", data=np.array([[s[0], s[1]] for s in sample_ids], dtype=np.int32))
...
def main():
    ...
    all_results = []
    sample_ids = []
    error_idx = set(np.random.choice(len(all_samples), min(5, len(all_samples)), replace=False))
    all_errors = []
    ...
    for i, (scene_id, sim_id) in enumerate(tqdm(all_samples, desc="Processing")):
        try:
            ...
            if i in error_idx:
                ...
                all_errors.append({"sample": (scene_id, sim_id), "n_nodes": len(result["grid"]), "errors": errs})
    ...
    print(f"Nodes: {min(node_counts)}-{max(node_counts)}, mean={np.mean(node_counts):.0f}")
    ...
    for e in all_errors:
        ...
        for vn, err in e["errors"].items():
            print(f"  {vn}: L1={err['l1']:.4f}, L2={err['l2']:.4f}, Linf={err['linf']:.4f}")
