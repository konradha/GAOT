...
def create_animation(orig, fixed, sample_idx, scene_id, sim_id):
    ...
    fig.suptitle(f"Sample {sample_idx} (scene={scene_id}, sim={sim_id})", fontsize=14, fontweight='bold')
    ...
    def update(frame_idx):
        ...
        if len(fluid_tri_remapped) > 0 and len(fluid_pts) > 0:
            ...
            ax_orig_mesh.scatter(fluid_pts[:, 0], fluid_pts[:, 1], s=0.5, c='blue', alpha=0.5)
            
            ax_orig_vx.tripcolor(triang, fluid_vx, cmap='RdBu_r', vmin=vmin, vmax=vmax, shading='flat')
        
        ax_orig_mesh.set_title(f"Original Mesh - {len(fluid_pts)} pts, {len(fluid_tri_remapped)} tri")
        ...
        ax_fixed_vx.scatter(fixed_pts[:, 0], fixed_pts[:, 1], c=fixed_vx, 
                           s=2, cmap='RdBu_r', vmin=vmin, vmax=vmax)
        
        ax_diff_mesh.scatter(fluid_pts[:, 0], fluid_pts[:, 1], s=1, c='blue', alpha=0.5, label='Original')
        ax_diff_mesh.scatter(fixed_pts[:, 0], fixed_pts[:, 1], s=1, c='red', alpha=0.5, label='Fixed')
        if frame_idx == 0:
        ...
        if len(fluid_pts) > 0 and len(fixed_pts) > 0:
            ...
            if close_mask.sum() > 0:
                ...
                diff_lim = max(abs(np.percentile(diff, 5)), abs(np.percentile(diff, 95)))
                if diff_lim < 1e-6:
                    diff_lim = 1.0
                
                ax_diff_vx.scatter(fixed_pts[close_mask, 0], fixed_pts[close_mask, 1], 
                                  c=diff, s=2, cmap='coolwarm', vmin=-diff_lim, vmax=diff_lim)
