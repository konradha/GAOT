...
TRAIN_TAUS = [1, 3, 5, 15, 30]
MAX_DIRECT_TAU = 30
AR_STEP = 30
EVAL_TAUS = [1, 5, 15, 30, 50, 250]
ALPHA = 0.1
...
def train_epoch(model, data, train_idx, stats, optimizer, device):
    ...
    der_mean = {tau: stats["der_mean"][tau].to(device) for tau in TRAIN_TAUS}
    der_std = {tau: stats["der_std"][tau].to(device) for tau in TRAIN_TAUS}
    ...
    for _ in pbar:
        ...
        taus = np.random.choice(TRAIN_TAUS, size=TIME_PAIRS_PER_FORWARD)
        t_ins = np.random.randint(0, N_TIMESTEPS - max_tau, size=TIME_PAIRS_PER_FORWARD)
        ...
        for t_in, tau in zip(t_ins, taus):
            ...
            target_np = (deriv - der_mean[tau].cpu().numpy()) / der_std[
                tau
            ].cpu().numpy()
            u_in_list.append(u_in_np)
            u_out_list.append(u_out_np)
        ...
        target = torch.from_numpy(np.stack(target_list)).to(device)
        tau_tensor = torch.tensor(taus, dtype=torch.float32, device=device)

        u_in_norm = (u_in - u_mean) / u_std

        tau_norm = (tau_tensor - tau_mean) / tau_std
...
def evaluate(model, data, sample_indices, stats, device, max_iters=500):
    ...
    for _ in pbar:
        ...
        tau = int(np.random.choice(TRAIN_TAUS))
        t_in = np.random.randint(0, N_TIMESTEPS - max_tau)
        t_out = t_in + tau
        nc = fields[(sample_idx, 0)].shape[0]

        u_in = torch.from_numpy(fields[(sample_idx, t_in)]).unsqueeze(0).to(device)
        u_out = torch.from_numpy(fields[(sample_idx, t_out)]).unsqueeze(0).to(device)
        mask = torch.from_numpy(masks[(sample_idx, t_in)]).unsqueeze(0).to(device)

        u_in_norm = (u_in - u_mean) / u_std
        deriv = (u_out - u_in) / tau
        target = (deriv - der_mean[tau]) / der_std[tau]

        tau_norm = (tau - tau_mean) / tau_std
        tau_feat = torch.full((1, nc, 1), tau_norm, device=device, dtype=torch.float32)
        ...
        pred_deriv = pred.squeeze(0) * der_std[tau] + der_mean[tau]
...
def ar_rollout(model, data, sample_indices, stats, device):
    ...
    def get_der_stats(tau):
        if tau in der_mean:
            return der_mean[tau], der_std[tau]
        closest = min(TRAIN_TAUS, key=lambda t: abs(t - tau))
        return der_mean[closest], der_std[closest]
    ...
    for sample_idx in tqdm(sample_indices, desc="AR Rollout", leave=False):
        ...
        for eval_tau in EVAL_TAUS:
            ...
            u_gt = torch.from_numpy(fields[(sample_idx, eval_tau)]).to(device)

            if eval_tau <= MAX_DIRECT_TAU:
                mask = torch.from_numpy(masks[(sample_idx, 0)]).to(device)
                u_curr_norm = (u_t0 - u_mean) / u_std

                tau_norm = (eval_tau - tau_mean) / tau_std
                tau_feat = torch.full(
                    (1, nc, 1), tau_norm, device=device, dtype=torch.float32
                )
                ...
                while remaining > 0:
                    ...
                    mask = torch.from_numpy(masks[(sample_idx, t_curr)]).to(device)
                    u_curr_norm = (u_curr - u_mean) / u_std

                    tau_norm = (step_tau - tau_mean) / tau_std
                    tau_feat = torch.full(
                        (1, nc, 1), tau_norm, device=device, dtype=torch.float32
                    )
            ...
            rel_l2 = (
                torch.sqrt(((u_pred - u_gt) ** 2).sum()) / torch.sqrt((u_gt**2).sum())
            ).item()
...
def main():
    ...
    logging.info(f"Training taus: {TRAIN_TAUS}, AR step: {AR_STEP}")
    ...
    config = build_config()
    model = GAOT(input_size=6, output_size=4, config=config).to(device)
    ...
    for tau in EVAL_TAUS:
        if tau in rollout:
            r = rollout[tau]
            method = "direct" if tau <= MAX_DIRECT_TAU else f"AR({AR_STEP})"
            logging.info(
    ...
    logging.info(
        "  +50:  baseline0 uses 50 AR steps, baseline1 uses 2 AR steps (30+20)"
    )
    logging.info(
        "  +250: baseline0 uses 250 AR steps, baseline1 uses 9 AR steps (30*8+10)"
    )
